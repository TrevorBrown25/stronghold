    ...
    <section className="flex flex-col gap-3 rounded-3xl bg-white/70 p-4 shadow-lg">
      ...
      <div className="grid gap-2 md:grid-cols-2">
        {EDICTS.map((option) => (
          <label
            key={option.value}
            className={`flex cursor-pointer flex-col gap-1 rounded-2xl border px-3 py-2 text-sm transition ${
              selected === option.value ? "border-accent bg-white" : "border-transparent bg-white/60"
            }`}
          >
      ...
      <button
        onClick={handleApply}
        className="w-fit rounded-full bg-accent px-4 py-2 text-sm font-semibold text-white transition hover:bg-accent-dark"
      >
        Apply Edict
      </button>
      {edictTurn === turn && (
